...
def preprocess_image(image_url):
    try:
        ...
        _, thresh = cv2.threshold(cp.asnumpy(gray), 155, 255, cv2.THRESH_BINARY)
        
        
        return cp.asarray(thresh)  # Return as CuPy array
...
# Set OCR config to assume a single uniform block of text
    # Convert CuPy array back to NumPy array for pytesseract
    image_np = cp.asnumpy(image)
    raw_text = pytesseract.image_to_string(image_np, config=config)
